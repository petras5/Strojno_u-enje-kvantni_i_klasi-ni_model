...
with open(r'adelie.csv', newline='') as csvfile:
    ...
    for pingvin in adelie:
        pom_array = [int(pingvin['Culmen_Length(mm)']),int(pingvin['Culmen_Depth(mm)']),int(pingvin['Flipper_Length(mm)']),int(pingvin['Body_Mass(g)'])]
        features.append(pom_array)
        labels=np.append(labels,int(pingvin['Sex']))

print("Features: ",features)
print("Labels: ",labels)
featureNames=['Culmen Length (mm)','Culmen Depth(mm)','Flipper Length(mm)','Body Mass(g)']
